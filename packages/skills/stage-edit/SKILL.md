...
3. **Generate each beat FROM the plan, then record its `produced_path`.** Size each narration line to its `target_sec` before calling TTS; run `ovs speak` per line, save the mp3 under `project/assets/narration/line-XX.mp3`, and write that line's `produced_path`. If the words do not fit at a natural pace, SHORTEN that line in the plan — never speed up past natural or let it run long/short. Coverage must span ~0→clip-end, not stop at the halfway mark.
...
1. **OCR the on-screen text — preferred, cheapest, no extra cost when available.** `ovs ocr` is the intended local OCR surface. In this open-source build it may report that OCR is not available yet; when it does, fall back immediately to frame extraction.
