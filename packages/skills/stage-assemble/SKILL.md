...
- **compose** → `stage-compose`: build a small visual-only manifest-owned composition for `spec.kind` (title card, lower-third, stat card, captions) under `project/compositions/<id>/` → run `ovs draft project/compositions/<id> --out project/parts/<id>.mp4 --quality draft --report project/reports/<id>-compose-report.json`. Pass `--not-opening` for every compose segment that is not the production's delivered opening: the cover family (cover contract, rendered cover checks, the hook label) belongs to the whole video's frame 0, and cover-blocking a middle segment stalls the line over a poster it will never be. Blank-frame checks still run — a blank first frame on a middle segment is a visible gap at the cut. This keeps compose segments on the same manifest/source/check/video-QA path as standalone COMPOSE while still letting the assembler own narration and loudness.
