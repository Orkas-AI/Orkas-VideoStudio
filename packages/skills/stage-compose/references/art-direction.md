...
- `references` + `reference_fidelity`: required for every concrete reference image/video, with reproduce/edit/guide intent, roles, composition-local path, preserve/may-change boundary, target scenes, layout/temporal anchors, and observable paired-frame comparison criteria.
