...
**The resolved frame must be what renders at t=0**: entrances animate FROM a visible state, never from `opacity: 0` — not on the scene, and not on any single element that carries its promise (the `data-role="title"`, the `data-cover-hero`, a declared signal). Every composition is frame-0 sampled — including each AUTO segment — so a scene that fades in captures blank and fails QA before anyone sees it, and hiding only the title still leaves frame 0 without the promise. The rule binds to the elements that carry the cover, not only to their container. A deliberate fade-from-black opening is the one exception, and it needs the user to have asked for it.
...
- Treat frame 0 as a dedicated cover. It needs one dominant topic-specific hero and at least two visible content signals from the actual video. QA reads the declared signals off the visible frame-0 copy, so a signal you actually put on screen counts; add `data-cover-signal="<exact content_signals value>"` only when the signal is carried by an element with no readable text of its own, and mark the hero with `data-role="visual" data-cover-hero` — never the title, generic background, or decoration. **Signals must say something the headline does not**: a `content_signals` entry that only restates the headline is reported as headline-only and does not count as a second signal. The headline itself may run across consecutive `data-role="title"` lines. QA reports rendered hooks and hero bounds as advisories, not blockers — design the cover to this standard because it is what makes the video open well, not because a checker will bounce you.
...
6. Judge the returned keyframes rather than only frame 0 — via the economical frame pass (contact sheet as the index; full scale for the cover, QA-named frames, and risky cells). There is no score: judge what is visibly broken in a specific frame. Compare reference layout anchors and protected axes side-by-side; for video references compare declared source time ranges to target-scene motion/timing. Judge the requested reproduce/edit/guide outcome, never the reference's origin.
